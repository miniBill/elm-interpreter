...
import TestUtils exposing (evalExpect, evalTest_, list)
    ...
    describe "Initialization"
        [ Test.skip <|
            fuzz defaultSizeRange "initialize" <|
                \size ->
                    evalExpect ("Array.toList <|Array.initialize " ++ String.fromInt size ++ " identity")
                        List
                        (Array.toList <| Array.initialize size Int)
        , let
            size : Int
            size =
                1056
        ...
        --         1056
        --   in
        --   evalTest_ ("Array.initialize " ++ String.fromInt size ++ " identity")
        --     (array Int)
        --     (Array.initialize size identity)
        -- , fuzz defaultSizeRange "push" <|
        --     \size ->
        --         List.foldl Array.push Array.empty (List.range 0 (size - 1))
